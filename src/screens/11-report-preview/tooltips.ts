...
export const T11 = {
  snapshotStatus:
    '報告快照狀態：確認本報告引用的 Screen 09 結果是否仍與目前 Scenario、Solver、08 分析及 thermal limits 一致。',
  reportReadiness:
  ...
  keepTableTogether:
    '避免此章節被分割到兩頁：放不下時整段移到下一頁。長表格與圖表章節（關鍵元件、熱網路摘要、瓶頸元件熱網分析）本來就會自動接續分頁，因此不受此設定影響，會從目前頁面剩餘空間開始排。',
  snapshotSource: '此報告章節所引用的 Screen 09 snapshot 與來源分析版本。',
  sourceScreen: '此章節數據最初來自哪個分析頁，例如 Screen 07、08、09 或 10。',
  languageMode: '設定報告內容使用英文或中英雙語；不影響應用程式 UI 的語言規則。',
  prepareForExport:
    '建立給 Screen 11 使用的 report export payload；本頁不會真正生成 PDF 或其他檔案。',
  exportPayload:
  ...
  overallStatus:
    '來自 Screen 09 快照的整體熱狀態；本頁只引用，不重新判定（FAIL 也不會阻擋報告產出）。',
  pageCount: '依目前章節與版面設定推估的頁數；實際頁數以 11 Export Center 的輸出為準。',
  ...
  criticalRowCount:
    '報告中列出的關鍵元件數量；排序與數值皆沿用 Screen 09，不重新計算。預設為「All」列出全部；若選 Top 5 / Top 10，表格下方會註明共有幾個元件被省略。',
  bottleneckTopN: 'Bottleneck 章節列出的候選數；資料來自 Screen 08，不重新排名。',
  includeHistogram:
    '嵌入既有的直方圖快照；本頁不會重新分箱，也不會重新計算百分位。',
} as const;
