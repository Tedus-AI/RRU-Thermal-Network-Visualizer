...
export const T10 = {
  ...
  improvementActions:
    '所有處於 WARNING 或 FAIL 的受監控元件，依餘裕由小到大排列。展開後列出「該元件在 08 已儲存的調整分析」中每一段被調整的熱阻，以及要達成該調整可以改動哪些參數與目標值；參數為即時依儲存的調整量重算，不是複製下來的快照。若某元件沒有已儲存的調整分析，本頁不會憑空提出改善方案，只提供前往 08 建立的入口。'
  ,
